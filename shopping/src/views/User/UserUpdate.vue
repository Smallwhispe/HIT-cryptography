<template>
  <el-container class="home_container">
    <el-header class="home_header">
      <div class="home_title">拼夕夕-用户编辑</div>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put('http://172.24.218.142:8181/user/update',_this.ruleForm).then(function (resp) {
            if(resp.data.code == 0){
              _this.$alert(_this.ruleForm.username+'修改成功', '', {
                confirmButtonText: '确定',
                callback: action => {
                  _this.$router.push('/userDefault')
                }
              });
            }
          })
  ...
  created() {
    const _this = this
    axios.get('http://172.24.218.142:8181/user/findById/'+_this.$route.query.id).then(function (resp) {
