<template>
  <el-container class="home_container">
    <el-header class="home_header">
      <div class="home_title">拼夕夕-用户后台数据</div>
...
export default{
  ...
  methods:{
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://172.24.218.142:8181/user/search',{params:_this.ruleForm}).then(function (resp) {
    ...
    page(currentPage){
      ...
      if(_this.ruleForm.value == ''){
        axios.get('http://172.24.218.142:8181/user/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
          _this.tableData = resp.data.data.data
          _this.total = resp.data.data.total
        })
      } else {
    ...
    edit(row){
      this.$router.push('/userUpdate?id='+row.id)
    },
    del(row) {
      const _this = this
      this.$confirm('确认删除【'+row.username+'】吗？', '提示', {
        ...
        axios.delete('http://172.24.218.142:8181/user/deleteById/'+row.id).then(function (resp) {
          if(resp.data.code==0){
  ...
  created() {
    const _this = this
    axios.get('http://172.24.218.142:8181/user/list/1/'+_this.pageSize).then(function (resp) {
